...
async def on_command_error(ctx, error):
    if isinstance(error, commands.ArgumentParsingError):
        await ctx.send("Please enter message correctly!")
...
async def on_message(message):
    ...
    if channel in all_channels:
        ...
        #TODO: Question do I have to put this in a function to use error handling?
        if user_message.lower() == "hello" or user_message.lower() == "hi":
            ...
            await message.channel.send(f"Here is you IP Address: {ec2_metadata.public_ipv4}, Your EC2 Region: {ec2_metadata.region}, and EC2 Instance ID: {ec2_metadata.instance_id}")
            return
